...
- **playlist** - Detailed settings for playlist.
  ...
  - **banners** - An array containing the information about playing banner static images. If this information is not present, no banners should be displayed.
      - **filename** - Filename, including file extension, of the image to be displayed.
      ...
      - **type** -  Specifies in which part of the fueling transaction the image should display.
        - Values
          - prefueling
          - postfueling
  - **schedules** - Contains the playlist details broken up by date.
    - **segmentId** - A unique identifier provided by GSTV for each date of the playlist. We will expect this in notifications related to this date of the playlist.
    - **date** - A String that specifies the date the playlist is active.  The format will be `year-month-day`.
    - **loops** - An array that contains all the loops to be played for the day.
      - **loopNumber** - Specifies the order in which this loop should be played in the sequence of loops. Each day will have multiple loops. Each loop should be played sequentially and repeated once the last loop has been played.
      - **assets** - An array that specifies all the assets to be played in this loop.
        - **assetId** - A string assigned to the media asset by GSTV. This identifier should be included in the ProofofPlay packet coming from Invenco to GSTV.
        ...
        - **type** - The type of video.
	      	- Values
						...
						- brandedMessaging
						- rpa
        - **start** - Date the video should begin playing. The format will be `year-month-day`.
        - **end** - Date the video should end displaying. The format will be `year-month-day`.
        ...
        - **dayparts** - An array that specifies specifc times of the day that a video should play. If no daypart is present, the video should play without regard to time.
          - **start** - Local site time (24 hour time) in which the asset should start playing if daypart is present. The format will be 'hours:mins'.
          - **end** - Local site time (24 hour time) in which the asset should stop playing if daypart is present. The format will be 'hours:mins'.
